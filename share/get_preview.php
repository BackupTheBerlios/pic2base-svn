...
//###############################################################################################################################
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
		...
		SWITCH($form_name)
		{
			...
			ELSE
			{
			//echo htmlentities("ï¿½bergebene Werte: Breite: ".$lat.", Lï¿½nge: ".$long.", Hï¿½he: ".$alt.", Umkreis: ".$radius." m, Ort. ".$ort.", Umkreis: ".$radius2." ".$einheit2)."<BR>";
			}
			
			//annï¿½hernde Berechnung des Toleranzfeldes aus dem Radius:
			//geogr. Breite: WInkeldifferenz je m Abweichung: 0,000008999280058ï¿½
		...
		{
			$content .=$mp;		//Bezugspunkt einfï¿½gen
			...
			FOR($i_winkel=0; $i_winkel<'361'; $i_winkel++)
			{
				...
				$lat = $lat_mittel + $y;
				//echo $lat."<BR>";
				//Lï¿½nge (unter Zulassung einer leichten Ellipse)
		...
		ELSE
		{
			FOR($i5=0; $i5<$num5; $i5++)
			{
				...
				//Prï¿½fung, ob der Punkt in einem KREIS-Bereich um das Zentrum herum liegt:
...
//###################################################################################
		...
		IF($desc1 !=='')
		{
			...
			IF($bewertung !== '6')
			{
				...
				$stat_ref = $stat_all.") AND ($table2.loc_id <>'0' OR $table2.loc_id <>'')$krit2 ORDER BY $table14.DateTimeOriginal";
			}
			ELSE
			{
				$stat_all = $statement;
				$stat_ref = $stat_all.") AND ($table2.loc_id <>'0' OR $table2.loc_id <>'') ORDER BY $table14.DateTimeOriginal";
			}
		...
		$statement = "$stat_all) ORDER BY $table14.DateTimeOriginal";
		break;
//###################################################################################	
		CASE 'exif':
