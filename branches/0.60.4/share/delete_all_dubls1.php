...
<?php 
//###############################################################
//wird beim loeschen aller Dubletten aus der DB verwendet
//###############################################################
...
//	echo "User-ID: ".$user_id."<BR>";
	...
	IF($method == 'all')
	{
		...
		$result0 = mysql_query("SELECT * FROM $table21");
	}
	ELSEIF($method == '')
	{
		//Variante, wenn die Dublettenpruefung von einem normalen User aus dem Bearbeiten-Menue gestartet wurde:
		$result0 = mysql_query("SELECT * FROM $table21 WHERE user_id = '$user_id'");
	}
			
	$num0 = mysql_num_rows($result0);
	IF($num0 > 0)
	{
//		echo $num0." Dubletten wurden gefunden.<BR>";
		FOR($i0=0; $i0<$num0; $i0++)
		{
			$new_pic_id = mysql_result($result0, $i0, 'new_pic_id');
			//Die Bild-Daten werden ermittelt:
			$result1 = mysql_query( "SELECT * FROM $table2 WHERE pic_id = '$new_pic_id'");
			@$num1 = mysql_num_rows($result1);
//			echo "in Tabelle pictures gefundene Eintr&auml;ge: ".$num1."<BR>";
			IF($num1 == '1')
			{
