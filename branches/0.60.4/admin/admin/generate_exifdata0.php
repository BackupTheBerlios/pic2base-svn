...
<p id='kopf'>pic2base :: Meta-Daten-Extraktion <span class='klein'>(User: ".$c_username.")</span></p>
...
<center>
Status der Meta-Daten-Extraktion
...
FOR($note='1'; $note<'6'; $note++)
{
	...
	FOR($i5='0'; $i5<$num5; $i5++)
	{
		...
		{
			//nur wenn es noch keinen Eintrag in der pictures-Tabelle gibt, wird die Erfassung ausgefuehrt:
				...
				IF($pos3 != '' AND $X == -1)
				{
					...
					IF($pos4 != '' AND $pos5 != '')
					{
						...
						IF (!in_array($fieldname, $tab_fieldname))
						{
							...
							//echo "Fehler beim Ergaenzen des Feldes: ".mysql_error(),"<BR>";
						}
						ELSE
						{
							//Wenn das Feld in der Tabelle ist, wird Feldtyp und Feldlaenge geprueft:
							//Pruefung Feldtyp:
							...
							IF(($ist_feldlaenge < $soll_feldlaenge) OR (($ist_feldtyp !== $soll_feldtyp) AND ($soll_feldtyp !== 'int')))
							{
								...
								//echo "Feldtyp muss fuer ".$fieldname." von ".$ist_feldtyp." auf ".$fieldtype." geaendert werden oder<BR>";
								...
								//echo "Fehler beim aendern der Feldlaenge: ".mysql_error(),"<BR><BR>";
							}
						}
					}
					$X = 1;
