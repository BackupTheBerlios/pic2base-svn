...
$result2 = mysql($db, "SELECT MAX(YEAR(DateTimeOriginal)) AS MAX_DTO, MIN(YEAR(DateTimeOriginal)) AS MIN_DTO FROM $table14 WHERE YEAR(DateTimeOriginal) <> '0000'");
...
$runtime_sum = 0;
//Bestimmung der Jahrgänge, in denen Bilder entstanden sind:
$result1 = mysql($db, "SELECT DISTINCT YEAR(DateTimeOriginal) AS DTO FROM $table14 WHERE YEAR(DateTimeOriginal) <> '0000' ORDER BY YEAR(DateTimeOriginal)");
$num1 = mysql_num_rows($result1);
FOR($i1 = '0'; $i1<$num1; $i1++)
{
	$D = mysql_result($result1, $i1, 'DTO');
	//Die einzelnen "Jahrgänge" werden ermittelt:
	//Zur Laufzeitoptimierung wird kontrolliert, ob in dem betreffenden Jahr überhaupt Bilder vorhanden sind:
	$start2 = microtime();
	//$result3a = mysql($db, "EXPLAIN SELECT * FROM $table2 WHERE note = '2'");
	$result3 = mysql($db, "SELECT DISTINCT $table14.DateTimeOriginal, $table14.pic_id, $table2.note FROM $table14 INNER JOIN $table2 ON ($table14.pic_id = $table2.pic_id AND $table14.DateTimeOriginal LIKE '$D-%' AND $table14.DateTimeOriginal <> '0000-00-00 00:00:00' AND $table2.$stat)");
	echo mysql_error();
	/*$end2 = microtime();
	...
	IF ($num3 > 0)
	{
		$jahr = date('Y', strtotime(mysql_result($result3, $i3, 'DateTimeOriginal')));
		$modus='recherche';
		...
		ELSE
		{
			$s_m = $jahr;		//Anzeige-Modus: Jahresübersicht
			$info_nr = '1';
		}
		...
		IF(substr($show_mod,0,4) == $jahr)
		{
			...
			//Die einzelnen Monate im gewählten Jahr werden ermittelt:
			FOR($i_M='1'; $i_M<='12'; $i_M++)
			{
				...
				$result9 = mysql($db, "SELECT DISTINCT YEAR(DateTimeOriginal), MONTH(DateTimeOriginal) FROM $table14 WHERE (YEAR(DateTimeOriginal) = '$jahr' AND MONTH(DateTimeOriginal) = '$i_M' AND DateTimeOriginal <> '0000-00-00 00:00:00')");
				IF(mysql_num_rows($result9) > 0)
				{
					$result4 = mysql($db, "SELECT DISTINCT $table14.DateTimeOriginal, $table14.pic_id, $table2.note, $table2.pic_id FROM $table14 INNER JOIN $table2 ON ($table14.pic_id = $table2.pic_id AND $table14.DateTimeOriginal LIKE '$jahr-$i_M%' AND $table14.DateTimeOriginal <> '0000-00-00 00:00:00' AND $table2.$stat) ORDER BY $table14.DateTimeOriginal");
				...
				IF($num4 > '0')
				{					
					$result5 = mysql($db, "SELECT DISTINCT $table14.DateTimeOriginal, $table14.pic_id, $table2.note, $table2.pic_id FROM $table14 INNER JOIN $table2 ON ($table14.pic_id = $table2.pic_id AND $table14.DateTimeOriginal LIKE '%$jahr-$i_M%' AND $table14.DateTimeOriginal <> '0000-00-00 00:00:00' AND $table2.$stat) ORDER BY $table14.DateTimeOriginal");
					$num5 = mysql_num_rows($result5);
					//echo $num5." Bilder im Monat ".$i_M." im Jahr ".$jahr."<BR>";
					
					$month_number = date('m', strtotime(mysql_result($result5, $i5, 'DateTimeOriginal')));
					echo "<TR id='kat'>
					<TD id='kat1'>&#160;&#160;&#160;&#160;&#160;";	
					$modus = 'recherche';
					...
					IF(substr($show_mod,0,4) == $jahr AND substr($show_mod,5,2) !== $month_number)
					{
						...
						$s_m = $jahr."_".$month_number;	//$show_mod: Anzeige-Modus für Tages-Anzeige
					...
					ELSEIF(substr($show_mod,0,4) == $jahr AND substr($show_mod,5,2) == $month_number)
					{
						...
						IF($jahr == $y AND $month_number == $m)
						{
							...
							FOREACH($dat_arr AS $datum)
							{
								...
								$zeit_t = $T.".".$zeit;
								echo "<TR id='kat'>
								<TD id='kat1'> &#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;".$aufn_dat."</TD>";
								//Ermittlung der an diesem Tag gemachten Bilder:
								
								$result12 = mysql($db, "SELECT DISTINCT $table14.DateTimeOriginal, $table14.pic_id, $table2.note, $table2.pic_id FROM $table14 INNER JOIN $table2 ON ($table14.pic_id = $table2.pic_id AND $table14.DateTimeOriginal LIKE '%".$aufn_DAT."%' AND $table14.DateTimeOriginal <> '0000-00-00 00:00:00' AND $table2.$stat) ORDER BY $table14.DateTimeOriginal");
...
$result7 = mysql($db, "SELECT $table14.DateTimeOriginal, $table14.pic_id, $table2.note, $table2.pic_id FROM $table14 INNER JOIN $table2 ON ($table14.pic_id = $table2.pic_id AND $table14.DateTimeOriginal = '0000-00-00 00:00:00' AND $table2.$stat)");
