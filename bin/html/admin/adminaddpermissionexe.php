<?
	...
	if ((hasPermission($c_username, 'addpermission')) AND ($description !== '') AND ($shortdescription !== ''))
	{
		...
		$result = mysql ($db,"INSERT INTO $table8 (description, shortdescription) VALUES ('".$description."', '".$shortdescription."')");
		$result1 = mysql($db, "SELECT * FROM $table8 WHERE description = '$description' AND shortdescription = '$shortdescription'");
		...
		$result2 =mysql($db, "SELECT * FROM $table9");
		$num2 = mysql_num_rows($result2);
		FOR($i2='0'; $i2<$num2; $i2++)
		{
			$group_id = mysql_result($result2, $i2, 'id');
			$result3 = mysql($db, "INSERT INTO $table6 (group_id, permission_id, enabled) VALUES ('$group_id', '$perm_id', '0')");
			
		}
		//in die Tabelle userpermissions wird für alle User die neue Berechtigung mit der Eigenschaft "nicht aktiviert" eingetragen:
		$result4 =mysql($db, "SELECT * FROM $table1");
		$num4 = mysql_num_rows($result4);
		FOR($i4='0'; $i4<$num4; $i4++)
		{
			$user_id = mysql_result($result4, $i4, 'id');
			$result5 = mysql($db, "INSERT INTO $table7 (user_id, permission_id, enabled) VALUES ('$user_id', '$perm_id', '0')");
