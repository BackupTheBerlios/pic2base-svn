...
ELSEIF(($loc_id !== '' AND $loc_id !== '0') AND $ort !== '' AND $location !== '')
{
	...
	flush();
	$result01 = mysql($db, "SELECT location FROM $table12 WHERE loc_id = '$loc_id'");
	$ort_alt = mysql_result($result01, $i01, 'location');
	//echo "alte Ortsbezeichnung: ".$ort_alt."<BR>";
	$result1 = mysql($db, "UPDATE $table12 SET longitude = '$long', latitude = '$lat', location = '$ort' WHERE loc_id = '$loc_id'");
	$result2 = mysql($db, "UPDATE $table14 SET GPSLongitude = '$long', GPSLatitude = '$lat', City = '$ort' WHERE pic_id = '$pic_id'");
...
ELSEIF(($loc_id == '' OR $loc_id == '0') AND $ort !== '' AND $location !== '')
{
	...
	flush();
	$ort_alt = '';
	$result1 = mysql($db, "INSERT INTO $table12 (longitude, latitude, altitude, location) VALUES ('$long', '$lat', '0.1', '$ort')");
	echo mysql_error();
	$result2 = mysql($db, "SELECT max(loc_id) FROM $table12");
	echo mysql_error();
	$loc_id = mysql_result($result2, $i2, 'max(loc_id)');
	$result3 = mysql($db, "UPDATE $table2 SET loc_id = '$loc_id' WHERE pic_id = '$pic_id'");
	echo mysql_error();
	$result4 = mysql($db, "INSERT INTO $table14 (GPSLongitude, GPSLatitude, GPSAltitude, City) VALUES ('$long', '$lat', '0.1', '$ort')");
	//Eintragung der Geo-Daten in den EXIF-Block des Originalbildes:
...
IF($ort_alt !== $ort)
{
	$result4 = mysql($db, "SELECT Caption_Abstract FROM $table14 WHERE pic_id = '$pic_id'");
	...
	//$result5 = mysql($db, "UPDATE $table2 SET description = '$description_neu' WHERE pic_id = '$pic_id'");
	$result5 = mysql($db, "UPDATE $table14 SET Caption_Abstract = '$description_neu' WHERE pic_id = '$pic_id'");
	//Vermerk im IPTC:Caption-Abstract-Tag:
	$desc_neu = htmlentities($description_neu);
	echo shell_exec($et_path."/exiftool -IPTC:Caption-Abstract='$desc_neu' ".$FN." -overwrite_original")."<BR>";
}

IF (mysql_error() == '')
{
	//für Testzwecke hält der Parameter das Fenster offen...
