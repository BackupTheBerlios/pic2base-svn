...
if (hasPermission($c_username, 'adminlogin') AND $users !== '')
{
	...
	$result0 = mysql($db, "SELECT username FROM $table1 WHERE id = '$id'");
	$benutzername = mysql_result($result0, $i0, 'username');
	echo $ftp_path."/".$benutzername;
	
	$result = mysql ($db,"UPDATE $table2 SET Owner = '$users' WHERE Owner = '$id'");
	echo "&Auml;ndere in pictures den Owner von $id auf $users<BR>";
	$result2 = mysql($db, "DELETE FROM $table1 WHERE id = '$id'");
	echo "L&ouml;scha alle Eintr&auml;ge des Users $id aus der Users-Tabelle.<BR>";
	$result3 = mysql($db, "DELETE FROM $table7 WHERE user_id = '$id'");
